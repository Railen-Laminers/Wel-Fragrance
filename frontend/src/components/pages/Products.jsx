...
// Import your product images (same as before)
...
export default function Products() {
    ...
    const [modalVisible, setModalVisible] = useState(false); // for enter/exit

    // Search & Filter state
    const [searchQuery, setSearchQuery] = useState('');
    const [selectedTag, setSelectedTag] = useState('All');

    // Animation state: load once
    ...
    useEffect(() => {
        // Small delay so the initial render is hidden, then fade in
    ...
    const closeModal = () => {
        ...
        // Wait for exit animation (300ms) then unmount
        setTimeout(() => {
        ...
        <>
            <div className="min-h-screen bg-transparent pt-20 sm:pt-24 md:pt-32 pb-16 sm:pb-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    {/* Header with fade-up */}
                    <div
                        className={`text-center mb-12 sm:mb-16 transition-all duration-700 ease-out transform ${
                    ...
                    {/* Search & Filter Bar – flexible layout */}
                    <div className="mb-10 sm:mb-12 flex flex-col sm:flex-row sm:items-center gap-4">
                        ...
                        <div className="flex-shrink-0 flex flex-wrap items-center gap-2 overflow-x-auto pb-1 scrollbar-thin scrollbar-thumb-old-gold/20 scrollbar-track-transparent">
                            {uniqueTags.map((tag) => (
                                <button
                                    key={tag}
                                    onClick={() => setSelectedTag(tag)}
                                    className={`font-jost text-[10px] sm:text-xs tracking-[0.15em] uppercase px-4 py-1.5 border transition-all duration-300 whitespace-nowrap ${
                                        selectedTag === tag
                    ...
                    {/* Product Grid – each card fades up with staggered delay */}
            ...
            {isModalMounted && selectedProduct && createPortal(
                ...
                >
                    <div
                        className={`relative max-w-4xl w-full max-h-[90vh] bg-warm-white dark:bg-dark-teal border border-old-gold/20 shadow-2xl overflow-y-auto transition-all duration-300 ease-out transform ${
